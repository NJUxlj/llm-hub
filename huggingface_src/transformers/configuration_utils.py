...
class PretrainedConfig(PushToHubMixin):
    ...
    def __getattribute__(self, key):
        '''
        这段代码的主要功能是检查要访问的属性名是否存在于 attribute_map 中，
        如果存在，则使用 attribute_map 中映射的属性名来替代原属性名进行访问。
        '''
    ...
    def __init__(self, **kwargs):
        ...
        # 使用 kwargs.pop(key, default) 方法从 kwargs 中获取参数，如果参数不存在则使用默认值。
    ...
    def name_or_path(self) -> str:
        '''
        注解的作用：
            现在，你可以通过 obj.name_or_path 来访问这个属性，而不是 obj.name_or_path()
        '''
    ...
    def _set_token_in_kwargs(kwargs, token=None):
        ...
        kwargs：一个字典，包含传递给 from_pretrained 方法的关键字参数。
        token：可选参数，默认为 None，表示身份验证令牌
        
        作用： 设置huggingface的登录凭证
        """
