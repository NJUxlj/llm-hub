...
class GenerationMixin:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        # 功能：这段代码的目的是找出模型在生成过程中允许使用的参数，
        #   以便后续检查用户传入的 model_kwargs 中是否有未被使用的参数。
        unused_model_args = []
        
        # 使用 inspect.signature 函数获取 self.prepare_inputs_for_generation 方法的参数签名。
            # 通过 .parameters 属性获取参数签名中的所有参数。
        model_args = set(inspect.signature(self.prepare_inputs_for_generation).parameters)
        # `kwargs`/`model_kwargs` is often used to handle optional forward pass inputs like `attention_mask`. If
        # `prepare_inputs_for_generation` doesn't accept them, then a stricter check can be made ;)
        if "kwargs" in model_args or "model_kwargs" in model_args: # 说明 prepare_inputs_for_generation 方法接受可变参数
            model_args |= set(inspect.signature(self.forward).parameters) # |= 是集合的并集操作，用于合并两个集合
